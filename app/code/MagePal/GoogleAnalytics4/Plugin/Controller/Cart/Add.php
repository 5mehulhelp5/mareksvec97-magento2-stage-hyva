...
namespace MagePal\GoogleAnalytics4\Plugin\Controller\Cart;
...
class Add
{
    ...
    protected function addTrackingData(ProductInterface $product, array $responseArray)
    {
        ...
        if ($this->messageManager->hasMessages()) {
            $noticeMessages = $this->messageManager->getMessages()->getItemsByType('notice');
            ...
            $errorMessages = $this->messageManager->getMessages()->getItemsByType('error');

            /** @var Notice $message */
            foreach ($errorMessages as $message) {
                $string = __('You can only buy this product in increments of');
                if (strpos($message->getText(), $string->getText()) !== false) {
                    $this->enhancedEcommerceSession->setGenericEvent(
                        $item,
                        'addToCartItemInvalidQtyIncrements'
                    );
                }
            }
        }
