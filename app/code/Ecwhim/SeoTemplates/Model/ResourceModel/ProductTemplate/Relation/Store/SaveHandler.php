    ...
    protected function processDelete(array $storeIds, int $templateId): void
    {
        if ($storeIds) {
            $this->templateResource->getConnection()->delete(
                $this->templateResource->getTable(AddProductTemplateStoreTable::TABLE_PRODUCT_TEMPLATE_STORE),
                [
                    ProductTemplateInterface::TEMPLATE_ID . ' = ?'           => $templateId,
                    AddProductTemplateStoreTable::COLUMN_STORE_ID . ' IN(?)' => $storeIds
                ]
            );
        }
    }
    ...
    protected function processInsert(array $storeIds, int $templateId): void
    {
        if ($storeIds) {
            ...
            foreach ($storeIds as $storeId) {
                $data[] = [
                    ProductTemplateInterface::TEMPLATE_ID         => $templateId,
                    AddProductTemplateStoreTable::COLUMN_STORE_ID => $storeId
                ];
            }
