...
class Popup extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    {
        $customerAllPage = '(' . implode(",", $this->customerAccountAll) . ')';
        $handleFilter = "t1.handle = 'default'";
        foreach ($handleList as $handle) {
            $handleFilter .= " OR t1.handle = '" . $handle . "'
            OR ((t1.handle = 'customer_account' AND '" . $handle ."' in " . $customerAllPage . " )
            OR (t1.handle = 'multishipping_checkout' AND '" . $handle ."' like '%multishipping_checkout%'))";
        }
