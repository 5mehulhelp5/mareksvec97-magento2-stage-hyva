...
define([
    ...
    $.widget('mage.productGallery', $.mage.productGallery, {
        ...
        _initDialog: function () {
			...
			$dialog.on('change', '[data-role="image-filename-overwrite"]', function (e) {
                var target = $(e.target),
                    targetName = target.attr('name'),
                    filename = target.val(),
                    imageData = $dialog.data('imageData');

                this.element.find('input[type="hidden"][name="' + targetName + '"]').val(filename);

                imageData.filename_overwrite = filename;
            }.bind(this));
