...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function getSortOrder($sortOrderString)
    {
        ...
        $ids = explode(',', trim(trim((string) $sortOrderString), ','));
    ...
    public function determineOptions($blockOptions)
    {
        ...
        if (0 === $categoryPageXml
            ...
            $options['disable_ids']  = trim(
                (string) $this->getConfig(
                    'apptrian_subcategories/category_page/disable_ids'
                ),
                ','
            );
        ...
        $options['image_width']            = $this->getConfig(
            'apptrian_subcategories/' . $pageType . '/image_width'
        );
        
        $options['image_height']            = $this->getConfig(
            'apptrian_subcategories/' . $pageType . '/image_height'
        );
    ...
    public function getCategories($data)
    {
        ...
        $attributesToSelect = ['name', 'url_key', 'url_path', 'image',
            'description', 'meta_description', 'meta_title','homepage_description','homepage_title'];
            
        if ($o['image'] == 'thumbnail') {
            $attributesToSelect[] = 'apptrian_thumbnail';
        }
        ...
        // category_ids field is empty
                        ...
                        if ($pageType == 'category_page') {
                            if ($catParam) {
                                ...
                                $currentCategory = $this->currentCategory->getCategory();
            					if ($currentCategory->getLevel()==3) {
            						$categoryId = $currentCategory->getParentId();
            						$result['current'] = $currentCategory->getId();
            					} else {
            ...
            // Get categories array from collection
            $categories = $this->getCategoriesFromCollection($collection, '', $disabledIds);
        }
    ...
    /**
     * Based on provided Category Collection and optionally sort order, returns sorted array of categories.
     *
     * @param \Magento\Catalog\Model\ResourceModel\Category\Collection $collection
     * @param string $sortOrder
     * @param array $disabledIds
     * @return array
     */
    public function getCategoriesFromCollection($collection, $sortOrder = '', $disabledIds = [])
    {
        $categories = [];
        
        if ($sortOrder != '') {
            $sort = explode(',', (string) $sortOrder);
    ...
    public function categoryToArray($c)
    {
        ...
        $category['homepage_description'] = $this->getHomepageDescription($c);
        $category['homepage_title'] = $this->getHomepageTitle($c);
        $category['id'] = $c->getId();
    ...
    public function getDescription($category)
    {
        ...
        return trim((string) $categoryDescription);
    }

    public function getHomepageDescription($category)
    {
        $homepageDescription = $category->getData('homepage_description');
        
        if ($homepageDescription) {
            return $this->catalogHelperOutput
                ->categoryAttribute($category, $homepageDescription, 'homepage_description');
        }


        return '';  // Ak nie je nastavený popis kategórie pre domovskú stránku, vrátime prázdny reťazec.
    }

    public function getHomepageTitle($category)
    {
        $homepageTitle = $category->getData('homepage_title');
        
        if ($homepageTitle) {
            return $this->catalogHelperOutput
                ->categoryAttribute($category, $homepageTitle, 'homepage_title');
        }
        
        // Ak nie je nastavený homepage_title, vrátime názov kategórie.
        return $this->escaper->escapeHtml($category->getName());
    }
    ...
    public function getImage($category)
    {
        ...
        if ($imageAttribute == 'image') {
            ...
            $image = $category->getApptrianThumbnail();
        } else {
            $image = '';
        }
    ...
    {
        $pool = explode(',', (string) $categoryIds);
    ...
    {
        $disableIds = $this->options['disable_ids'];
        
        return explode(',', (string) $disableIds);
    }
    ...
    public function getExcludedIds()
    {
        ...
        return explode(',', (string) $excludeIds);
    }
    ...
    {
        $idFiltered = (int) preg_replace('/[^0-9]+/', '', strip_tags((string) $id));
