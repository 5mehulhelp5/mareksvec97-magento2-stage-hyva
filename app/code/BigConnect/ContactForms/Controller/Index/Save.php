<?php
namespace BigConnect\ContactForms\Controller\Index;
...
class Save extends \Magecomp\Quickcontact\Controller\Index\Save {
    ...
    public function __construct(
        ...
        parent::__construct($context, $uploaderFactory, $adapterFactory, $filesystem, $helperdata, $senderResolver, $storeManager, $transportBuilder, $state, $logger, $quickcontactFactory, $jsonResultFactory);
    }
    ...
    public function execute() {
		...
		if (isset($data['files'])) $data['attachment'] = implode(',', $data['files']);
        ...
        if ($datasave) {
			...
			try {
				...
				$templateVars = [
					'subject' => $subject." ".$quickcontact->getCustomername(),
					'customerName' => $quickcontact->getCustomername(),
					'customerEmail' => $quickcontact->getCustomeremail(),
					'customerPhone' => $quickcontact->getCustomerphone(),
				...
				$attachment = $quickcontact->getAttachment();
				if (!empty($attachment)) {
				...
				$transport = $this->transportBuilder->setTemplateIdentifier($templateId, $storeScope)
					...
					->setFrom($fromEmail)
					->addTo($toEmail)
					->setReplyTo($quickcontact->getCustomeremail()) // Přidáno
					->getTransport();
